...
import RegeneratePasswordModal from "@/components/data-collectors/RegeneratePasswordModal";
...
export default function DataCollectors() {
  ...
  const filteredCollectors = useMemo(() => {
    ...
    return collectors.filter((collector) => {
      ...
      const supervisorName = collector.supervisor?.name?.toLowerCase() || "";
      const supervisorEmail = collector.supervisor?.email?.toLowerCase() || "";
      return (
        name.includes(query) ||
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setCollectors((current) =>
        current.filter((collector) => collector.id !== deleteTarget.id)
      );
      setDeleteTarget(null);
    } catch (err) {
      setError(
        err.response?.data?.message ||
          "Failed to delete data collector. Please try again."
      );
    } finally {
      setDeletingId(null);
    }
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="mt-1 text-[13px] text-ink-soft">
              Onboard and manage data collectors assigned to supervising officers.
            </p>
          </div>
          ...
          <div className="px-5 py-4 border-b border-line">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search by name, email, officer..."
                  value={searchTerm}
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-end gap-2 flex-wrap">
                          ...
                          >
                            {deletingId === collector.id ? "Deleting..." : "Delete"}
                          </button>
                        </div>
                      </td>
                    </tr>
                    ...
                    <td colSpan={5} className="px-5 py-12 text-center">
                      ...
                      <p className="mt-1 text-[12px] text-ink-soft">
                        {searchTerm
                          ...
                          : "Add your first data collector to get started."}
                      </p>
                    </td>
                  </tr>
                )}
