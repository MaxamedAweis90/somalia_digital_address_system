...
export default function EditNeighborhood() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const [neighRes, distRes] = await Promise.all([
          getNeighborhoodById(id),
          getDistricts(),
        ]);

        const neigh = neighRes.data.data;
  ...
  const validateForm = () => {
    ...
    if (!formData.code.trim()) {
      newErrors.code = "Neighborhood code is required";
      isValid = false;
    } else if (formData.code.trim().length < 2) {
      newErrors.code = "Neighborhood code must be at least 2 characters";
      isValid = false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setServerError(error.response?.data?.message || "Failed to update neighborhood");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-[400px] flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-sm text-ink-soft">Loading neighborhood information...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-bg font-sans">
      <div className="px-4 sm:px-6 lg:px-5 pt-5 pb-10">
        ...
        <div className="flex items-center gap-2 text-[11px] font-medium text-ink-soft mb-6">
          ...
          <span className="text-gray-400">›</span>
          <span className="text-ink font-semibold">Edit Neighborhood</span>
        </div>
        ...
        <div className="mb-6">
          ...
          <p className="mt-1 text-[13px] text-ink-soft">
            Update neighborhood information and boundary parameters.
          </p>
        </div>
        ...
        <div className="w-full max-w-[900px] bg-white border border-line rounded-xl shadow-card-sm overflow-hidden">
          <div className="px-5 py-5 border-b border-line">
            ...
            <p className="mt-1 text-[13px] text-ink-soft">
              Update the official details for this neighborhood.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit}>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-line">
                <div>
                  ...
                  <p className="text-[12px] text-ink-soft mt-0.5">
                    Enable or disable this neighborhood in the system
                  </p>
                </div>
            ...
            <div className="flex items-center justify-end gap-3 px-5 py-4 bg-[#FBFBFC] border-t border-line">
              ...
              <button
                ...
                "
              >
                {saving ? "Updating..." : "Update Neighborhood"}
              </button>
            </div>
          </form>
        </div>
