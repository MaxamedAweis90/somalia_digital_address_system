...
const zoneSteps = [
  {
    title: "Open your zone assignment",
    detail: "From My Assignments, open a Define Zones task for your neighborhood.",
  },
  {
    title: "Add each zone",
    detail: 'Click Add, then give every zone a name and code (e.g. "Zone 01" / Z01).',
  },
  {
    title: "Draw boundaries on the map",
    detail:
      "Use the map tools to draw one polygon per zone. Stay inside the gray neighborhood boundary when it is shown.",
  },
  {
  ...
  },
  {
    ...
    detail:
      "When all zones are complete, submit for approval. An admin will review and publish them to the registry.",
  },
];
...
const addressSteps = [
  {
    ...
    detail: "From My Assignments, open a Register Addresses task for your assigned zone.",
  },
  {
  ...
  },
  {
    ...
    detail:
      "Select an address, then click inside the blue zone boundary to set its location. Drag the marker to adjust.",
  },
  {
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div className="rounded-lg border border-violet-200 bg-violet-50/50 p-4">
            <div className="mb-3 flex items-center gap-2 text-violet-800">
              ...
              <p className="text-[13px] font-semibold">Define Zones</p>
            </div>
            <StepList steps={zoneSteps.slice(0, 3)} />
          </div>
    ...
    <div className="space-y-6">
      <div className="rounded-xl border border-line bg-white p-5 shadow-card-sm">
        ...
        <p className="mt-2 text-[13px] text-ink-soft leading-relaxed">
          ...
          administrator prepares neighborhoods and zones, then assigns you tasks. You record
          boundaries and properties on the map, save drafts, and submit for approval. Official
          DAC codes are only issued after admin approval — not when you draw a zone.
        </p>
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div className="rounded-lg bg-bg border border-line px-4 py-3">
            ...
            <p className="mt-1 text-[11px] text-ink-soft">
              Draw zones or place one pin per building you verify in the field.
            </p>
          </div>
          <div className="rounded-lg bg-bg border border-line px-4 py-3">
            ...
            <p className="mt-1 text-[11px] text-ink-soft">
              Zones must stay inside the neighborhood. Pins must stay inside the zone.
            </p>
          </div>
          <div className="rounded-lg bg-bg border border-line px-4 py-3">
            ...
            <p className="mt-1 text-[11px] text-ink-soft">
              After approval, zones go live and addresses receive sequential DAC codes.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <div className="rounded-xl border border-line bg-white p-5 shadow-card-sm">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="text-[15px] font-semibold text-ink">Task 1: Define Zones</h3>
          </div>
          <StepList steps={zoneSteps} />
        </div>
        ...
        <div className="rounded-xl border border-line bg-white p-5 shadow-card-sm">
          ...
          <p className="mt-4 rounded-lg bg-amber-50 border border-amber-200 px-3 py-2 text-[11px] text-amber-900">
            Important: the system does not know how many buildings are in a zone. You must add
  ...
  const steps =
    type === "REGISTER_ADDRESSES"
      ? [
          ...
          "Click inside the blue zone on the map to place the GPS pin.",
          "Save Draft, then Submit for Approval when finished.",
        ]
      : [
          "Click Add for each zone you need to define.",
          "Enter zone name and code, then draw its boundary on the map.",
          "Keep polygons inside the gray neighborhood boundary when shown.",
          "Save Draft, then Submit for Approval when finished.",
        ];
