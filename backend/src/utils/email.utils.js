...
export const sendOtpEmail = async (toEmail, otpCode, { name } = {}) => {
  console.log(`\n=================================================`);
  console.log(`🔑 [AUTH OTP CODE] User: ${toEmail} | Code: ${otpCode}`);
  console.log(`=================================================\n`);

  if (!process.env.SMTP_USER || !process.env.SMTP_HOST) {
    console.log("[EMAIL BYPASS] SMTP is not configured. Use the OTP code logged above to sign in.");
    return;
  }
...
export const sendLoginSuccessEmail = async (toEmail, { name, device, os, browser, ip, time } = {}) => {
  ...
  try {
    ...
    console.error("Failed to send login success email via SMTP:", err.message);
  }
};